const API_KEY = '2UQXif_l_DRFs5YJw51OdSUGki1t_NBrvUGkSWyp1hY';
...
searchInput.addEventListener('input', () => {
 ...
 fetch(`https://trefle.io/api/v1/plants/search?token=${API_KEY}&q=${plantName}&complete_data=true`)
 .then((response) => response.json())
 .then((data) => {
 plantResults.innerHTML = '';
 data.data.forEach((plant) => {
 const option = document.createElement('option');
 option.value = plant.common_name;
...
searchBtn.addEventListener('click', () => {
    ...
    fetch(`https://trefle.io/api/v1/plants/${plant.id}?token=${API_KEY}`)
    .then((response) => response.json())
    .then((data) => {
    ...
    <ul>
    <li>Scientific Name: ${data.data.scientific_name || ''}</li>
    <li>Genus: ${data.data.genus.name || ''}</li>
    <li>Family: ${data.data.family.name || ''}</li>
    <li>Year: ${data.data.year || ''}</li>
    <li>Bibliography: ${data.data.bibliography || ''}</li>
    </ul>
    `;
    if (data.data.family_common_name !== null) {
    div.innerHTML += `<p>Family Common Name: ${data.data.family_common_name}</p>`;
    }
    if (data.data.observations) {
    div.innerHTML += `<p>Observations: ${data.data.observations}</p>`;
    }
    if (data.data.main_species && data.data.main_species.vegetable !== null) {
    div.innerHTML += `<p>Vegetable: ${data.data.main_species.vegetable ? 'Yes' : 'No'}</p>`;
    }
    if (data.data.edible_part) {
    div.innerHTML += `<p>Edible Part(s): ${data.data.edible_part}</p>`;
    }
    if (data.data.main_species && data.data.main_species.edible !== null) {
    div.innerHTML += `<p>Edible: ${data.data.main_species.edible ? 'Yes' : 'No'}</p>`;
    }
    if (data.data.medicinal) {
    div.innerHTML += `<p>Medicinal Properties: ${data.data.medicinal}</p>`;
    }
    if (data.data.duration) {
    div.innerHTML += `<p>Duration: ${data.data.duration}</p>`;
    }
    if (data.data.growth_habit) {
    div.innerHTML += `<p>Growth Habit: ${data.data.growth_habit}</p>`;
    }
    if (data.data.growth_rate) {
    div.innerHTML += `<p>Growth Rate: ${data.data.growth_rate}</p>`;
    }
    if (data.data.toxicity) {
    div.innerHTML += `<p>Toxicity: ${data.data.toxicity}</p>`;
    }
   
    // Images
    const images = data.data.main_species.images;
    if (images) {
    let imagesHTML = '<h3>Images</h3>';
    let hasImages = false;
    if (images.flower && images.flower.length > 0) {
    hasImages = true;
    imagesHTML += '<h4>Flower</h4>';
    images.flower.forEach((image) => {
    imagesHTML += `<img src="${image.image_url}" alt="Flower" />`;
    });
    }
    if (images.fruit && images.fruit.length > 0) {
    hasImages = true;
    imagesHTML += '<h4>Fruit</h4>';
    images.fruit.forEach((image) => {
    imagesHTML += `<img src="${image.image_url}" alt="Fruit" />`;
    });
    }
   ...
   if(hasImages){
    ...
    div.innerHTML +=
   `<h3>Flower:</h3><ul><li><strong>Color:</strong>${colorStr}</li><li><strong>Conspicuous:</strong>${conspicuousStr}</li></ul>`;
    }
    ...
    if(growth){
      ...
      if(growth.minimum_temperature && growth.minimum_temperature.deg_c && growth.minimum_temperature.deg_f){
        hasGrowthInfo=true;
        growthHtml+=`<li><strong>Minimum Temperature:</strong>${growth.minimum_temperature.deg_c}°C (${growth.minimum_temperature.deg_f}°F)</li>`
      }
      if(growth.maximum_temperature && growth.maximum_temperature.deg_c && growth.maximum_temperature.deg_f){
        hasGrowthInfo=true;
